...
v2.0 起交付仍是**单一 skill zip** —— 没有 wheel，没有 wheel / 全包 `pip install`，没有 setuptools。从 skill.sh 一键安装到 `~/.claude/skills/mneme/`，agent 直接用。OKF 核心零依赖；L1（SQLite FTS5）也是零依赖；可选语义召回层推迟到 v2.1。
...
> 本文件即 Karpathy 思想中的 **schema 层**：它让 agent 成为有纪律的 wiki 维护者，而非泛泛的聊天机器人。
...
`init / lint / reindex / dream` 是 agent 在后台跑的确定性 CLI 子命令，不出现在用户叙事里。
...
```yaml
---
type: <Type name>            # REQUIRED（OKF 协议级 MUST）
...
okf_version: "0.1"           # 仅 bundle 根 index.md 可带
---
```
...
| 字段 | 角色 | 例子 |
|---|---|---|
| `type` | 文档角色（OKF 协议级 MUST） | `Concept` · `Reference` · `Summary` · `Source` · `Topic` |
...
- **Topic 页面**（如 `topics/llm-wiki.md`）是普通 OKF `Topic` 页面，承担"按主题聚合概念"的角色——agent 若需要阅读地图，写一份普通 `Topic` 页面即可，**不要**为每个 tag 镜像一份 `tags/<tag>.md`，维护成本高且容易腐化。
...
- **分层依赖**：L1（OKF bundle + okflib + sqlite3 + FTS5）保持 stdlib 零依赖。**L2（可选语义召回）推迟到 v2.1** —— v2.0 不引入、不打包、不自动装；用户用语义召回时再单独装。skill 不自带任何 `pip install` 动作。
- **disposable accelerator**：索引文件可删可重建；不是事实来源；删除后 wiki 仍然完整。
- **按需、无常驻服务**：所有 ingest / query / lint 走 host-agent 本地工具（Read / Write / Edit / Bash / Grep / Glob），skill 自身只提供 OKF 合规骨架与确定性 CLI（`mneme init / lint / reindex / search / dream`）；MCP 暂不实现。
...
- 不实现远程 / 云索引；只本地 sqlite3 + FTS5（L2 推迟到 v2.1）。
- 不定义固定的概念类型 taxonomy（`type` 值不集中注册）。
- 不规定存储 / 服务 / 查询基础设施（除参考实现）。
- 不替代领域 schema（Avro / Protobuf / OpenAPI）——OKF 引用它们，不吞并它们。
- 不引入 v2 converters（Word / PDF / PPT / Excel / 图片 / HTML）——v2.1 单独规划。
- v2 不实现 MCP server（CLI + skill 已覆盖）。

## 下一步（v2.1+）

1. **L2（可选语义召回）** —— 用户明确要求语义召回时再装；本仓库不打包、不默认开启。
2. **v2 converters**（Word / PDF / PPT / Excel / 图片 / HTML → md/csv）—— 薄封装 + lazy import + `mneme[converters]` extras。
3. **dream 完整审计 + 回填** —— 当前 `dream_audit` 只读骨架已经落地（见 SKILL.md）；完整相似度合并 / 归档 / 建议仍在 v2.1。
