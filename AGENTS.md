...
- **按需、无常驻服务**：所有 ingest / query / lint 走 host-agent 本地工具（Read / Write / Edit / Bash / Grep / Glob），skill 自身只提供 OKF 合规骨架与确定性 CLI（`mneme init / lint / reindex / search / dream / convert`）；`convert` 仅探测并调用已存在的外部可执行文件；MCP 暂不实现。
...
- 不内建文档解析器、不自动安装 converter；2.0.1 仅允许调用用户已有的 PDF/DOCX/PPTX 外部工具，其他格式与 OCR 能力另行规划。
