...
| **Disposable accelerator** | 可删除的导航缓存 | 不拥有事实 | `wiki/.mneme/fts.db` + `wiki/.mneme/l2.db`（显式启用后） |
...
- **分层依赖**：L1（OKF bundle + okflib + sqlite3 + FTS5）保持 stdlib 零依赖。3.x 的 L2 仅在用户显式执行一次 `reindex --l2` 时启用，依赖由用户自行安装；成功后模式持久化，日常 `search` / `reindex` 不依赖 agent 重复携带标志。skill 不自动执行 `pip install`、不静默回退索引模式。
- **disposable accelerator**：索引文件可删可重建；不是事实来源；删除后 wiki 仍然完整。
- **按需、无常驻服务**：dream / search 走 host-agent 本地工具（Read / Write / Edit / Bash / Grep / Glob），skill 自身只提供 OKF 合规骨架与确定性 CLI（`mneme init / lint / reindex / search / dream / convert`）；MCP 暂不实现。
...
- 不实现远程 / 云索引；只使用本地 SQLite，默认 FTS5、显式可选 L2。
...
1. **L2 质量评估** —— 只改进显式可选的候选召回，不改变 Markdown 权威性或 dream 审批契约。
2. **converter 扩展** —— 在不内建解析器、不自动安装的前提下评估更多格式与 OCR 适配。
3. **dream 审计增强** —— `dream_audit` 保持只读；合并 / 归档 / 回填仍由 agent 预览并等待用户批准。
