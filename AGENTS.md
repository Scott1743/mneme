...
v2.2：完成 workflow 契约迁移；v3.2：保留默认 FTS5 + 显式可选 L2
```
...
| **Disposable accelerator** | 可删除的导航缓存 | 不拥有事实 | `wiki/.mneme/index.db`（L1 或显式 L2） |
...
- **分层依赖**：L1（OKF bundle + okflib + sqlite3 + FTS5）保持 stdlib 零依赖。3.x 的 L2 仅在用户显式使用 `--l2` 时启用，依赖由用户自行安装；skill 不自动执行 `pip install`、不静默切换索引模式。
