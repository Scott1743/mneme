...
`scripts/mneme/` 子目录是真正的 Python 包（skill-first 交付的标准布局）。`scripts/mneme.py` 是给 skill.sh 用户用的入口 shim（`python3 ~/.claude/skills/mneme/scripts/mneme.py <subcmd>`）。需要语义判断的工作流继续由宿主 agent 按 `SKILL.md` 执行。wiki 仍走**外部 bundle 模型**——`sample-bundle/` 只是测试夹具，真 wiki 路径由 `~/.config/mneme/config.toml` 的 `bundle_path` 指定。
