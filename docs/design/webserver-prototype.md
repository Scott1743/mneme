...
| **stdlib 零依赖**（test_zero_dep.py 强制） | flask/fastapi 全部出局 | 服务端只用 `http.server.ThreadingHTTPServer` + `json` + `urllib`；前端**零 CDN、无运行时 npm**——HTML + 原生 ES2017 JS + 手写 CSS，并随 Skill zip 固定打包 MIT 许可的 G6 5.1.1 浏览器文件。Python/SQLite 核心仍为 stdlib 零依赖，中国网络受限环境下也能离线工作。 |
...
3. **应用壳内嵌，浏览器图库严格白名单。** 前端应用是 `webui.py` 里的 `INDEX_HTML` 字符串（HTML+CSS+JS 一体），`GET /` 直接返回；唯一静态路径是固定的 `/assets/g6-5.1.1.min.js`，对应 Skill 内 vendored MIT 资产。没有通用静态目录、CDN 或构建步骤。
...
└── vendor/             # 固定版本 G6 浏览器包、MIT LICENSE、第三方声明
```
...
graph.db 的全部节点/边由随 Skill 离线分发的 AntV G6 5.1.1 渲染，ForceAtlas2 负责大图布局。节点不会再被截断成固定数量：完整 Graph 铺在独立世界坐标里，默认相机只显示局部，支持拖拽平移、滚轮缩放、适配全图、全屏和小地图定位；筛选只改变可见性，不重新打乱布局。当前邻域通过弱化无关节点和边保持全局方位。工作台仍可按基础层/富化层、节点种类、关系谓词和关键词切片；页面、标签和 agent 实体采用不同形状。选中节点或关系会显示邻接关系、置信度、证据与来源页面，浏览页也反向显示当前页面的 Graph 上下文。Graph 缺失时明确提示首次构建，富化层为空时不会伪装成已有数据。
...
P1 不依赖 P2；P2 每一个写入端点单独评审。服务端与 Python/SQLite 核心不引入第三方依赖；浏览器端只允许审计过、固定版本、随 zip 分发的资产，不引入构建步骤或运行时下载。
...
3. **UI 形态（已解决）**：应用壳继续内嵌在 `webui.py`；只为固定版本 G6 开放一个严格白名单静态路径，不建立通用 `webui/` 静态目录。
