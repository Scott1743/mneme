...
| `/api/page?path=` | GET | 单页：frontmatter dict + raw markdown + 出链/入链 | 文件 + graphlib |
| `/api/dream` | GET | dream 只读审计报告（hard rules / writer rules / graph 健康） | `dream.dream_audit` |
| `/api/graph` | GET | 节点/边列表（P3 图谱页用） | graphlib |
| `/api/reindex` | POST | 重建 fts/graph 缓存（disposable，幂等） | `cmd_reindex` 同款逻辑 |
...
graph.db 的节点/边用约 150 行原生 Canvas 力导向布局渲染，无第三方图库；节点按 type 着色，点击节点跳浏览页。P1 先放占位页显示节点/边统计。
...
| **P3** | 图谱力导向可视化；log.md 时间线视图；`--host 0.0.0.0` 局域网分享（如真有需求） | 同 P2 |
