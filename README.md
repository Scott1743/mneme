...
`wiki/.mneme/graph.db`、`wiki/.mneme/fts.db` 和可选的 `wiki/.mneme/l2.db` 只是导航缓存。删掉索引不会丢失任何知识，Markdown bundle 始终完整。
...
- 最新版：[mneme-4.7.0.zip](https://github.com/Scott1743/mneme/releases/download/v4.7.0/mneme-4.7.0.zip)
...
v4.7 给 Agent 增加了 tag 与 Graph 富化的策展规约：文档角色放 `type`，来源身份放 `resource` 或 Source 链接，只有可跨页复用的主题切面才成为 tag，值得跨页查询的概念才成为实体，有原文证据的连接才成为语义关系。默认建议每页复用 1–3 个 tag（通常不超过 4 个）；需要富化时提取 3–6 个实体和 2–5 条语义关系，不手写由系统生成的 `mentions`。

`dream --json` 会只读报告单例 / 重复 tag、超预算页面、单例谓词和谓词分布，帮助 Agent 在预览阶段复用 bundle 已有词汇。这些都是 Mneme 写作建议，不是固定 taxonomy，也不会改变 OKF 的容错消费契约；清理或重命名现有词汇仍需用户明确批准。
...
`dream --json` 会报告 tag / 富化词汇健康度，并在 `graph.db` 存在时附带实体、关系、孤立实体、未解析页面与连通分量统计；审计仍然完全只读。

`mneme serve` 的图谱工作台可以在**合并 / 基础 / 富化**三个层次间切换：基础层由 Markdown 页面、frontmatter tags 与页面链接确定性派生；富化层只展示经过 dream 预览批准后 ingest 的 agent 提取实体和关系，并保留置信度、证据与来源页面。工作台会一次载入完整 Graph，用 ForceAtlas2 把节点铺在独立世界坐标中；默认镜头只看可读的局部，可拖拽平移、滚轮缩放、用右下角小地图定位全局，也可适配全图、聚焦当前邻域、收起详情或进入全屏。固定版本的 AntV G6 浏览器资源随 Skill zip 离线分发，不使用 CDN，也不改变 Python/SQLite 核心的零依赖契约。v4.6.1 起页眉和浏览器标题会显示当前运行的 Skill 版本；v4.6.2 统一了 Web UI 与 lint 的孤儿页判定。页面浏览页也会反向展示当前页面的 Graph 上下文。
