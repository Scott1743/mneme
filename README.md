...
- 最新版：[mneme-4.6.2.zip](https://github.com/Scott1743/mneme/releases/download/v4.6.2/mneme-4.6.2.zip)
...
`mneme serve` 的图谱工作台可以在**合并 / 基础 / 富化**三个层次间切换：基础层由 Markdown 页面、frontmatter tags 与页面链接确定性派生；富化层只展示经过 dream 预览批准后 ingest 的 agent 提取实体和关系，并保留置信度、证据与来源页面。v4.6 新增有界概览、当前邻域、全量视图、返回历史与全屏浏览；v4.6.1 起页眉和浏览器标题会显示当前运行的 Skill 版本；v4.6.2 统一了 Web UI 与 lint 的孤儿页判定。页面浏览页也会反向展示当前页面的 Graph 上下文。
