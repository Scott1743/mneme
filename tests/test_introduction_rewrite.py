...
architecture (raw / wiki / skill / disposable accelerator) and describe
explicitly activated L2 as the third Hybrid retrieval leg.
"""
...
def test_public_pages_explain_v48_three_way_hybrid() -> None:
    readme = _read(README)
    intro = _read(INTRO)
    assert "Graph 0.75 / FTS5 0.10 / L2 0.15" in readme
    assert "Graph、FTS5 与 L2" in intro
...
def test_public_pages_explain_v47_curation_guidance() -> None:
    readme = _read(README)
    intro = _read(INTRO)
    assert "1–3 个 tag" in readme and "3–6 个实体" in readme
    assert "2–5 条语义关系" in readme and "`mentions`" in readme
    assert "重复或单例 tag" in intro and "漂移的关系谓词" in intro
