...
├── SKILL.md               # (planned) agent skill 载体：ingest / query / lint 工作流
├── .research/             # 立项前的研究资料与约束
│   ├── README.md          #   目录索引 + provenance + 检索方法
│   ├── constraints.md     #   蒸馏后的硬约束
│   ├── design-rationale.md#   Karpathy→OKF→skill 设计推演
│   ├── references.md      #   全部来源链接 + 生态地图
│   └── upstream/          #   上游规范原文（MIT，verbatim，勿改）
│       ├── OKF-SPEC.md
│       ├── OKF-README.md
│       └── karpathy-llm-wiki.md
├── references/            # (planned) skill 支撑文档（工作流详述、校验规则）
├── scripts/               # (planned) OKF 校验脚本（零依赖标准库）
├── wiki/                  # (planned) OKF bundle 本体 —— 知识 wiki
│   ├── index.md           #   bundle 根索引（渐进式展开入口）
│   ├── log.md             #   日期前缀的变更时间线
│   ├── sources/           #   raw sources（只读）
│   └── <concept>.md       #   概念页
└── .gitignore
```

`(planned)` 标记者为本阶段尚未实现，是后续工作。
