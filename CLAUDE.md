...
- **分层依赖**：L1（OKF bundle + okflib）保持 stdlib 零依赖；L2/L3 走 pip extras 隔离（`mneme[index]`、`mneme[agents]`、`mneme[all]`），base 安装不被拖重。
- **按需、无常驻服务**：L3 Strands agent（ingest/query/lint）按 CLI 调用执行；MCP 暂不实现（CLI+skill 已覆盖链入，未来按需再加）。
