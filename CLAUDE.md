...
│       │   ├── tools_helpers.py    #   通用路径 / slug 辅助（mneme.py 共用）
│       │   └── mneme.py            #   CLI 入口（argparse）
...
`scripts/` 内的 L2 索引、CLI 入口与辅助函数是 v2 新增；v2.1 起去掉了独立 agent 层（host-agent 直接驱动 skill）。wiki 仍走**外部 bundle 模型**——`sample-bundle/` 只是测试夹具，真 wiki 路径由 `~/.config/mneme/config.toml` 的 `bundle_path` 指定。PR3 将重构为真正的 `mneme` Python 包并通过 wheel 安装（详见 `docs/superpowers/plans/2026-07-12-mneme-0.3.0-implementation.md`）。
