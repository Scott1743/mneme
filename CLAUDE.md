...
- **载体**是一个通过 [skill.sh](https://skill.sh) 分发的 Claude Code agent skill —— `SKILL.md` 教会 agent 像维护源码一样 ingest / query / lint 一座 OKF 合规的 wiki。

一句话：**让 agent 在一个本地 Markdown 目录上，增量地编译、查询、体检一座 OKF 合规的知识 wiki。**

v1.1.0 起改为**单一 skill-first 交付**——不再发布 wheel；用户从 skill.sh 安装到 `~/.claude/skills/mneme/` 后直接用，OKF 核心零依赖、L2 语义搜索懒装（首次 search/reindex 时自动 pip install）。
...
- **分层依赖**：L1（OKF bundle + okflib）保持 stdlib 零依赖；L2 走 pip extras 隔离（`mneme[index]`、`mneme[all]`），base 安装不被拖重。
- **按需、无常驻服务**：所有 ingest / query / lint 走 host-agent 本地工具（Read / Write / Edit / Bash / Grep / Glob），skill 自身仅提供 OKF 合规骨架与可执行 CLI（`mneme init` / `reindex` / `search` / `lint`）；MCP 暂不实现（CLI+skill 已覆盖链入，未来按需再加）。
...
├── pyproject.toml                  # 开发/测试配置（pytest + 依赖注册；无 [project.scripts]）
├── .gitignore
├── skills/
│   └── mneme/                      # ← 唯一交付物（skill.sh layout，~/.claude/skills/mneme/）
│       ├── SKILL.md                #   英文版（权威）
│       ├── SKILL cn.md             #   中文版
│       ├── scripts/
...
│       │       ├── indexlib.py     #   L2 sqlite-vec + fastembed 索引库（懒装）
│       │       ├── config.py       #   ~/.config/mneme/config.toml 读写（tomllib + 手写 writer）
│       │       ├── tools_helpers.py
│       │       ├── toml_writer.py  #   手写 TOML writer（v1.1.0 新增，替换 tomli_w）
│       │       └── lazy_index.py   #   L2 懒装 + PEP 668 分支 + os.execvp 重启（v1.1.0 新增）
...
`scripts/mneme/` 子目录是真正的 Python 包（不是 v0.3.0–v1.0 的 `src/mneme/` 包布局）。`scripts/mneme.py` 是给 skill.sh 用户用的入口 shim（`python3 ~/.claude/skills/mneme/scripts/mneme.py <subcmd>`）。wiki 仍走**外部 bundle 模型**——`sample-bundle/` 只是测试夹具，真 wiki 路径由 `~/.config/mneme/config.toml` 的 `bundle_path` 指定。v1.0 readiness 报告里的 P0 "Delivery model is internally inconsistent" 由 v1.1.0 关闭（详见 `docs/superpowers/plans/2026-07-13-mneme-1.1.0-implementation.md`）。
...
2. **首次实际 ingest**：把 `/Users/scott1743/Desktop/佳都/飞书文档库/` 的 141 个 `.md` 作为 raw 素材，`mneme init ~/mneme-wiki` + `mneme ingest` 蒸馏成 OKF 概念页 + L2 索引。
