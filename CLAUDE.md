...
v2.0 起交付仍是**单一 skill zip** —— 没有 wheel，没有 wheel / 全包 `pip install`，没有 setuptools。从 skill.sh 一键安装到 `~/.claude/skills/mneme/`，agent 直接用。OKF 核心零依赖；L1（SQLite FTS5）也是零依赖；语义召回层推迟到 v2.1。
