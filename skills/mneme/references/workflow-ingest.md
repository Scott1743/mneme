...
7. **Reindex**: `Bash: python3 ~/.claude/skills/mneme/scripts/mneme.py reindex`. This rebuilds the local FTS5 candidate index.
8. **Validate**: `Bash: python3 ~/.claude/skills/mneme/scripts/mneme.py lint <bundle>` — must be 0 ERROR.

This release has no optional search dependencies and never downloads models.
