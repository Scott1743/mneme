...
- **Search:** `python3 ~/.claude/skills/mneme/scripts/mneme.py search <query> --json` embeds the question → sqlite-vec KNN top-k → joins chunk text + concept metadata → ranked results. `distance` is sqlite-vec's raw distance (for ranking, not a 0–1 similarity). First invocation triggers `ensure_index_deps()` (installs L2 deps + downloads model); subsequent calls use cached deps.
