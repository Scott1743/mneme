...
Helper:
```bash
Bash: python3 -m mneme --help
```
The `mneme` console command resolves the bundle the same way the CLI does
(it reads `~/.config/mneme/config.toml` and walks up for `okf_version`).

> **Skill-relative paths are no longer required.** v0.3.0 ships `mneme`
> as a console command; you do not need to know where the skill is
> installed. The references live under `<bundle>/references/` after a
> successful install of the wheel (or symlink-install).
...
1. `Bash: mneme init <path> [--config <cfg>]` (paths relative to cwd; if absent, `--config` defaults to `~/.config/mneme/config.toml`).
...
1. `Bash: mneme reindex [--config <cfg>]`
...
1. `Bash: mneme search "<query>" --json [--type <type>] [-k <limit>]`
...
6. `Bash: mneme reindex` (or `python3 -m mneme reindex`).
...
1. `Bash: mneme search "<question>" --json -k 10`
...
1. `Bash: mneme lint <bundle>` — read ERRORs (must fix) and WARNings.
2. (After PR2 lands `find_orphans`.) Read a sample of pages; look for contradictions / stale timestamps / missing cross-links.
...
`scripts/validate_okf.py` (validator — accessed via `mneme lint <bundle>` after install) · `references/workflow-ingest.md` · `references/workflow-query.md` · `references/workflow-lint.md` · `references/type-vocab.md` · `references/wiki-structure.md` · `references/index-design.md`.
