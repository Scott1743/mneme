...
ingest 前基线与 v4.1 重跑逐题一致（G: 1/—/4/—/—，G+L1: 1/—/1/—/—）。ingest 后 graph：193 entities（+47 llm）、519 relations（+91 llm）、1 个连通分量、0 orphan。

| Query | G 前 | G 后 | G+L1 前 | G+L1 后 |
|---|---|---|---|---|
| H01 gstack | 1 | 1 | 1 | 1 |
| H02 Claude Code 工作流 | — | **1 ✓** | — | **4 ✓** |
| H03 银行回单 | 4 | 4 | 1 | 1 |
| H04 录音（误标注） | — | —（正确） | — | — |
| H05 Hermes | — | **2 ✓** | — | **4 ✓** |
| **Recall@1** | 0.2 | **0.4** | 0.4 | 0.4 |
| **Recall@10** | 0.4 | **0.8** | 0.4 | **0.8** |
| **MRR@10** | 0.25 | **0.55** | 0.4 | **0.5** |
| **nDCG@10** | 0.286 | **0.612** | 0.400 | **0.572** |

v4.1 的全局 FTS5 并集保证 hybrid 不低于词法基线；agent extraction 进一步把 H02/H05 带入 top-10，但 Graph 与 FTS 的权重仍使其排在若干全局词法候选之后。
...
- **H02（Graph enrichment 补召回）**：v4.1 把「工作流」列为通用 seed，未 ingest 时 G 不返回候选；提取后 `Claude Code` 实体让期望页在 G rank 1、G+L1 rank 4。全局 FTS 候选仍在 hybrid 中保留，说明并集修复没有为了 Graph 命中牺牲词法召回。

## 6. 结论

1. **机制成立**：agent 提取 → `graph ingest` 能让正文实体进入图谱并使来源页可达。G Recall@10 0.4→0.8、nDCG@10 0.286→0.612；G+L1 Recall@10 0.4→0.8、nDCG@10 0.400→0.572。
2. **v4.1 排序修复有效但未解决数据稀疏**：种子已按 exact/prefix/description 分级，边权和高 degree tag 已降权，通用词 stoplist 已扩充。剩余瓶颈是 8/142 页的 extraction 覆盖率与 hybrid 权重校准。
